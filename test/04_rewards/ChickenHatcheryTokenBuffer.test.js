...
contract('ChickenHatcheryTokenBuffer', ([alice, bar, minter]) => {
    beforeEach(async () => {
        ...
        this.buffer = await ChickenHatcheryTokenBuffer.new(this.factory.address, bar, this.chicken.address, this.weth.address);
    ...
    it('should make CHKNs successfully', async () => {
        ...
        // After calling convert, bar should have CHKN value at ~1/6 of revenue
        ...
        await this.weth.transfer(this.chickenWETH.address, '10000000', { from: minter });
        await this.chickenWETH.mint(minter);
        assert.equal((await this.chickenWETH.balanceOf(this.buffer.address)).valueOf(), '16537');
        await this.buffer.convert(this.chicken.address, this.weth.address);
        assert.equal((await this.chicken.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.chickenWETH.balanceOf(this.buffer.address)).valueOf(), '0');
    });
});
